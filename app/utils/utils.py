...
def get_worker_id() -> int:
    ...
    # Convert pod name into a deterministic integer
    hash_value = hashlib.sha256(pod_name.encode()).digest()

    # SnowflakeGenerator typically needs a bounded worker ID
    return int.from_bytes(hash_value[:4], "big") % 1024
